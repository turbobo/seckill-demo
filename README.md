...
创建5000个用户
启动redis、rabbbitmq、项目，然后运行UserUtil。
优化
页面缓存
...
● 2.原子操作：Redis会将整个脚本作为一个整体执行，中间不会被其他命令插入，【判断和删除】是lua脚本执行的，要么全部成功，要么全部失败。
...
首先定义注解  ==》定义aop拦截器去拦截带RequestRateLimitAnnotation注解的接口 ==》controller接口上添加注解
...
秒杀面试题
● 为什么订单后就减库存，而不是付款减库存？
因为秒杀的意义就在于强盗商品，只要在一定时间内---15分钟内付款就行。
